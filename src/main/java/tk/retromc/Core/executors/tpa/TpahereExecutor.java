...
public class TpahereExecutor extends Applicator {
	public TpahereExecutor(Core i) {
		instance = i;
		isSelf = false;
		messagePath = "tpahere";
	}
	...
	public boolean executeCommand(CommandSender sender, Command cmd, String label, Player target, String args[]) {
		...
		if (target == (Player) sender) {
			sender.sendMessage(instance.getMessage("tpahere.usageMessage"));
			return true;
		}

		instance.tpaMap.put(targetId, new SimpleEntry(targetId, dispatcherId));

		sender.sendMessage(instance.getMessage("tpahere.send").replaceAll("\\$target", target.getName()));
		target.sendMessage(instance.getMessage("tpahere.recieve").replaceAll("\\$dispatcher", sender.getName()));
